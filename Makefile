...
golangci-lint-backend:
	docker run -ti --rm -v $(shell realpath ./backend):/app -w /app golangci/golangci-lint:v1.27.0-alpine golangci-lint run --modules-download-mode=readonly ./...

golangci-lint-notify:
	docker run -ti --rm -v $(shell realpath ./notify):/app -w /app golangci/golangci-lint:v1.27.0-alpine golangci-lint run --modules-download-mode=readonly ./...
